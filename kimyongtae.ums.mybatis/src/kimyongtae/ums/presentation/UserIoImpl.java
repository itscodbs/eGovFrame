package kimyongtae.ums.presentation;

import java.util.List;

import kimyongtae.ums.domain.User;
import kimyongtae.ums.domain.UserJob;
...
public class UserIoImpl implements UserIo{
	...
	public void listUsers(){
		...
		if(users.size()!=0) for(User user : users) Console.info(user);
		else Console.info("사용자가 없습니다."); 
	}
	...
	public void correctUser(){
		...
		if(userService.correctUser(user)) Console.info("사용자 정보 수정 성공.");
	...
	public void secede(){  // Delete
		...
		if(userService.secede(userNo)) Console.info("사용자 탈퇴 성공.");
